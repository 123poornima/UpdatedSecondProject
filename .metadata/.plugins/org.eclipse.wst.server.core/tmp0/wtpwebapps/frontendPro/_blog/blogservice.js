app.factory('BlogService',function($http){
	...
	blogService.addPost=function(blogPost){
		...
		return $http.post("http://localhost:8085/backendPro/blog",blogPost);
	}
	
	blogService.getBlogPosts=function(){
		console.log('getBlogposts in service')
		return $http.get("http://localhost:8085/backendPro/blog/list")
	}
	blogService.getBlogDetail=function(id){
		console.log('getBlogDetails')
		return $http.get("http://localhost:8085/backendPro/blog/get/"+ id)
	}
	
	blogService.addComment=function(comment){
		return $http.post("http://localhost:8085/backendPro/blog/comment",comment)
	}
	blogService.getComments=function(blogId){
		console.log('getcomments -- service')
		return $http.get("http://localhost:8085/backendPro/blog/getcomments/"+blogId)
	}
	return blogService;
})